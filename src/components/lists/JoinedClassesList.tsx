...
export const JoinedClassesList = () => {
  const pathname = usePathname();
  const { data: createdClasses, isLoading, isError } = useJoinedClasses();
  ...
  return (
    <div className="mt-2 space-y-4 w-full flex flex-col items-center">
      <div className="w-full flex flex-col items-center space-y-2">
        <p className="text-xs font-medium text-gray-600">Joined Classes</p>
        ...
        {createdClasses?.length
          ...
          : !isLoading && (
              <p className="text-sm text-gray-400">No classrooms joined</p>
            )}
      </div>
    </div>
  );
